...
def main():
    ...
    initial_data = [int(char) for char in data_string]

    phase_setting_combinations = list(itertools.permutations([0,1,2,3,4], 5))
    final_output_signal_combinations = {}

    for phase_setting_combination in phase_setting_combinations:
        output_signal = 0
        output_count = 1
        for phase_setting in phase_setting_combination:
            ...
            while i < len(data):
                ...
                if opcode == 1:
                    ...
                    if input_count == 1:
                        data[data[i+1]] = phase_setting
                        input_count += 1
                    elif input_count == 2:
                        data[data[i+1]] = input_signal
                        input_count += 1
                    i += 2
                elif opcode == 4:
                    output_signal = data[data[i+1]]
                    if output_count == len(phase_setting_combination):
                        final_output_signal_combinations[output_signal] = phase_setting_combination
                    output_count += 1
                    break
                elif opcode == 5:
                    first_param = data[i+1] if first_param_mode else data[data[i+1]]
    ...
    print(max(final_output_signal_combinations.keys()))    
